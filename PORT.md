...
| 14 | `decode_suffix` | engine/general_purpose/decode_suffix.rs:11-165 | Specialized to `RequireCanonical` + `decode_allow_trailing_bits = false` (the other two `DecodePaddingMode` arms and the trailing-bits bypass deleted). Iterator loop (`.iter().enumerate()`) rewritten as an index `while` loop. Output written by absolute index instead of `get_mut().ok_or(...)?` (see #3). `DecodeMetadata` return (decoded len + padding offset) reduced to the decoded length — the padding offset is internal plumbing not exposed by upstream's public `decode`. `debug_assert`s dropped. |
| 15 | `decode_slice` | engine/general_purpose/decode.rs:35-121 `decode_helper` | 8-symbol unrolled chunk loop (lines 46-88) dropped (see #13); the 4-symbol quad loop is kept as the single main loop. `chunks_exact().enumerate()` rewritten as an index `while` loop; sub-slice plumbing replaced by absolute indices. |
